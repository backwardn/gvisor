...
func (flow testFlow) header4Tuple(d packetDirection) header4Tuple {
	...
	if flow.isV4() {
		if d == outgoing {
			h = header4Tuple{stackAddr, testAddr, stackPort, testPort}
		} else {
			h = header4Tuple{testAddr, stackAddr, testPort, stackPort}
		}
		if flow.isMulticast() {
			h.dstAddr = multicastAddr
		} else if flow.isBroadcast() {
			h.dstAddr = broadcastAddr
		}
	} else { // IPv6
		if d == outgoing {
			h = header4Tuple{stackV6Addr, testV6Addr, stackPort, testPort}
		} else {
			h = header4Tuple{testV6Addr, stackV6Addr, testPort, stackPort}
		}
		if flow.isMulticast() {
			h.dstAddr = multicastV6Addr
		}
	}
	return h
}
...
func (flow testFlow) isV4() bool { return !flow.isV6() }

func (flow testFlow) isV6() bool {
	switch flow {
	case unicastV6, unicastV6Only, multicastV6, multicastV6Only:
		return true
	default:
		return false
	}
...
func (c *testContext) getPacketAndVerify(flow testFlow, checkers ...checker.NetworkChecker) []byte {
	...
	case p := <-c.linkEP.C:
		...
		// This is a packet that was sent out by calling Write().
		h := flow.header4Tuple(outgoing)
		checkers := append(checkers,
			checker.SrcAddr(h.srcAddr),
			checker.DstAddr(h.dstAddr),
			checker.UDP(
				checker.DstPort(h.dstPort)))
...
func (c *testContext) injectV6Packet(payload []byte, h *header4Tuple) {
	...
	xsum := header.PseudoHeaderChecksum(udp.ProtocolNumber, h.srcAddr, h.dstAddr, uint16(len(u)))
...
func testFailingWrite(c *testContext, flow testFlow, wantErr *tcpip.Error) {
	h := flow.header4Tuple(outgoing)
	writeDstAddr := flow.mapAddrIfApplicable(h.dstAddr)

	payload := buffer.View(newPayload())
	_, _, gotErr := c.ep.Write(tcpip.SlicePayload(payload), tcpip.WriteOptions{
		To: &tcpip.FullAddress{Addr: writeDstAddr, Port: h.dstPort},
	})
	if gotErr != wantErr {
	...
	for _, flow := range []testFlow{unicastV4, unicastV4in6, unicastV6, unicastV6Only, multicastV4, multicastV4in6, multicastV6, broadcast, broadcastIn6} {
		t.Run(fmt.Sprintf("flow:%s", flow), func(t *testing.T) {
			...
			if flow.isMulticast() {
				...
				if ep, err := p.NewEndpoint(0, tcpip.AddressWithPrefix{}, nil, nil, nil); err != nil {
					t.Fatal(err)
				} else {
